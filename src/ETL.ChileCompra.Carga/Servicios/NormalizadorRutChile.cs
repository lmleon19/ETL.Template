...
public sealed class NormalizadorRutChile
{
    ...
    private static ValorRut LimpiarInterno(string? rut)
    {
        ...
        {
            return new ValorRut(0, null, rut, false);
        }
        ...
        {
            return new ValorRut(0, digitoVerificador, rut, false);
        }

        digitoVerificador = string.IsNullOrWhiteSpace(digitoVerificador)
            ? null
            : digitoVerificador;
        bool esValido = EsRutValido(numero, digitoVerificador);

        return new ValorRut(numero, digitoVerificador, rut, esValido);
    }
